...
class RegisterController extends HomebaseController {
	...
	private function _do_mobile_register(){
	    ...
	    if($result){
	        ...
	        if($rst){
	        	if(session('?wechat_user_info')){
	        		$wechat_user_info = session('wechat_user_info');
	        		$data=array(
	        				'uid' => $rst['id'],
	        				'from' => 'weixin',
	        				...
	        				'wechat_user_options' => json_encode($wechat_user_info),
	        				'last_login_ip' => get_client_ip(0,true),
	        				'last_login_ip' => get_client_ip(0,true),
	        				'create_time' => date("Y-m-d H:i:s"),
	        				'last_login_time' => date("Y-m-d H:i:s"),
	            ...
	            if(session('?wechat_user_info')){
		            $this->success("注册成功！", U('Wechat/Index/index'));
	            }
