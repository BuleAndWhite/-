...
class AdminTermController extends AdminbaseController
{
    ...
    function index()
    {
        ...
        $result = $this->terms_model->where("term_id in ($map)")->order(array("path" => "asc"))->select();
...
//		$result = $this->terms_model->order(array("listorder"=>"asc"))->select();
        ...
        $tree->nbsp = '&nbsp;&nbsp;&nbsp;';
        foreach ($result as $r) {
            $r['str_manage'] = '<a href="' . U("AdminTerm/add", array("parent" => $r['term_id'])) . '">' . L('ADD_SUB_CATEGORY') . '</a> | <a href="' . U("AdminTerm/edit", array("id" => $r['term_id'])) . '">' . L('EDIT') . '</a>';
            if ($r['parent'] != 0) {
                $r['str_manage'] .= ' | <a class="js-ajax-delete" href="' . U("AdminTerm/delete", array("id" => $r['term_id'])) . '">' . L('DELETE') . '</a> ';
            }
...
//        $map = $map ? "5," . $map : 5;
//
//        $terms = $this->terms_model->where("term_id in ($map)")->order(array("path" => "asc"))->select();
    ...
    function add_post()
    {
        if (IS_POST) {
            if ($this->terms_model->create()) {
                if ($this->terms_model->add() !== false) {
                    F('all_terms', null);
                    $this->success("添加成功！", "AdminTerm/add_post");
