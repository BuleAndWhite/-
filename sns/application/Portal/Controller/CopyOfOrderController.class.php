...
// 		redirect('/admin');
    ...
    public function  three2(){
        ...
        if(isset($_REQUEST['piaoid'])){
            ...
            exit('<div style="width:40%;height:100%;padding:80px 20px 80px 20px;color:white;margin:auto;padding:auto">
...
$(".qupiao").click(function(){
        ...
        if(isset($_REQUEST['iscg'])&&$_REQUEST['iscg']=='yes2'){
           ...
           $key=1;
            foreach ($rs as $k=>$r)  {
                ...
                $str.='<td style="width:150px;position:relative" '.$posa.'  class="xp" value="p'.$key.'"  piaoid="'.$r["ticket_id"].'"  >
                ...
                foreach ($os as $k=>$p){
					...
					$str3='	
						...
						<div style="color:#CCC;font-size:12px;margin-top:20px">
								'.$r['comment'].'
						</div>
            ...
            exit('
                	<div style="width:40%;padding:80px 20px 80px 20px;color:white;margin:auto;padding:auto;">
		<div style="float:left">
			<div style="font-size:26px">
				在线订票
			</div>
	...
	<div style="color:#FFE599;MARGIN-TOP:20px;font-size:14px">
...
在线购票一旦交易成功，本公司不办理退票、换票、退款、返还打折票价业务，本公司不会因为天气（高温、下雨）等原因办理退票手续。
	...
	<script>
      ...
      $("#piaoid").val("");
           $("#body").html("    <div style=\'width:40%;padding:80px 20px 80px 20px;color:white;margin:auto;padding:auto;\'><div style=\'float:left\'><div style=\'font-size:26px\'>在线订票	</div><div>T: (021)64783333-2371	</div></div>	<div style=\'float:right;margin-top:5px\'><div><div style=\'float:left;cursor:pointer\' onclick=\'myorder()\'><img style=\'width:20px\' src=\'/themes/simplebootx/Public/images/fenlei.png\'  /></div><div style=\'float:left;margin-top:-1px;margin-left:10px;cursor:pointer\' onclick=\'myorder()\'>我的订单</div></div></div><div style=\'clear:both\'></div><div style=\'margin-top:20px\'><div>请选择门票类型</div><img src=\'/themes/simplebootx/Public/images//bg1.png\'  style=\'width:100%;margin-top:-20px\' /></div><div style=\'margin-top:10px;\'> <center ><div style=\'margin-top:10%\'><div onclick=\'one()\'><div><img src=\'/themes/simplebootx/Public/images/piao.png\'  /></div><div style=\'color:white\'>当日票</div></div><div onclick=\'one2()\'><div><img src=\'/themes/simplebootx/Public/images/piao.png\'  /></div><div style=\'color:white\'>预售票</div></div></div></center></div></div></div>");
            ...
            $count=M("order")->where("uid='".$user['id']."'")->order("create_time desc")->count();
                ...
                if($num>=$count){
                    ...
                    exit();
                        ...
                        $num2=$num;
                        $rs=M("order")->where("uid='".$user['id']."'")->order("create_time desc")->limit($num,1)->select();
                        foreach ($rs as $key =>$r){
                            $os=M("ticket")->where("ticket_id='".$r['ticket_id']."'")->find();
                            $rs[$key]['start_time']=$os['start_time'];
                            $rs[$key]['end_time']=$os['end_time'];
