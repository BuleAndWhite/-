...
class SlideController extends AdminbaseController{
	...
	function index(){
		...
		$slides=$this->slide_model->where(array("did"=>$usersModel['did']))->order("listorder ASC")->select();
	...
	function add_post(){
		if(IS_POST){
			if ($this->slide_model->create()) {
                $_POST['creator']=get_current_admin_id();
				$_POST['slide_pic']=$_POST['slide_pic'];

				if ($this->slide_model->add($_POST)!==false) {
