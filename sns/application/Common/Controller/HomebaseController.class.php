...
class HomebaseController extends AppframeController {
	...
	protected function weixin_check_user($mobile){
	...
	protected function  check_user(){
	    $user_status=M('Users')->where(array("id"=>session('uid')))->getField("user_status");
		if($user_status==2){
    ...
    /**
     * @date：2018年3月15日15:09:33
     * @param：活动推荐
     * @User：刘柏林
     */
	public function recommendedActivities(){
        //推荐活动
        $where = array("a.term_id = 2 and a.status=1  and b.review_id !=-1 and b.recommended = 1  ");
        $recommendedActivities = $this->term_relationships_model
            ->alias("a")
     ...
     * @User：刘柏林
     */
    public function recommendationCompetition(){
        //推荐活动
        $where = array("a.term_id = 1 and a.status=1  and b.review_id !=-1 and b.recommended = 1  ");
        $recommendationCompetition = $this->term_relationships_model
            ->alias("a")
            ->join(C('DB_PREFIX') . "posts b ON a.object_id = b.id")
            ->where($where)
            ->order("b.post_modified DESC")->select();
        return $recommendationCompetition;
    }
