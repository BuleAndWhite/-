...
class AdminbaseController extends AppframeController {
     ...
     *  排序 排序字段为listorders数组 POST 排序字段为：listorder
     */
    protected function _sortList($model) {
        if (!is_object($model)) {
            return false;
        }
        $pk = $model->getPk(); //获取主键名称
	...
	protected function init_token(){
		$id=$_SESSION['ADMIN_ID'];  
		$link = M('weixin_options_link')->where('uid='.$id)->order('is_use desc')->select();
		$tempstr = '';
		foreach ($link as $k => $v){
			$tempstr = $tempstr ? $tempstr.','.$v['mp_id'] : $v['mp_id'];
		}
		$map['id'] = array('in',$tempstr);
		$weixin_options_list = M ('weixin_options')->where($map)->order('FIND_IN_SET(id,"'.$tempstr.'")')->select();
		$token = get_token();
		if ($weixin_options_list[0]['app_wxid'] && ($token == '' || $token == '-1')) {
			session ( 'TOKEN', $weixin_options_list[0]['app_wxid'] );
		}
	}
	protected function check_token(){
		$token = get_token();
		if(empty($token) || $token == '-1'){
			$this->error('请先添加公众号！',U('WeixinPublic/index'));exit;
		}
	}
