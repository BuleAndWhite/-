...
class AlumniCircleController extends AppController
{
    ...
    public function postNews()
    {
        ...
        if(!empty($atList)){
            foreach ($atList as $k => $v){
                $atList[$k]=json_encode($v);
                $userId[$k]=$v['uid'];
            }
            $atList =json_encode($atList);
        }else{
            $atList='';
        }
        $sof_add = array(
            "uid" => $uid, "content" => json_encode($content,true), "type" => $type,"atList"=>$atList, "create_time" => date("Y-m-d H:i:s", time()),
        );
        //将数据插入朋友圈表
        ...
        //@好友的实时发送好友
        $sendFrom = M('user')->where(array('id'=>$uid))
            ->field("name,avatar,id uid")
            ->find();
        $sendData=[ 'content'=>"你的好友".$sendFrom['name']."@了你",'sendFrom'=>$sendFrom,'time'=>time()];
        $this->sendToWsServer('userAtList',[$userId],$sendData,'sendMessage');
        //资源,媒资是否为空
        if (!empty($media)) {
            //路径存储。关联起来
    ...
    public function myDynamicList()
    {
        ...
        $dynamic_list =M('Sof')->alias("s")->join("right join __USER__ as u on u.id = s.uid")->where($where)->field("s.id,u.avatar,s.uid,s.atlist,s.create_time,u.name,s.content,s.praise_num,s.click_num,s.reply_num,s.transfer_num,s.type")->page($page, $page_num)->order("s.create_time desc")->select();
        foreach ($dynamic_list as $k => $v) {
            ...
            //获取用户@的好友的数据
            if($v['atlist']!=null){
                $getNewAtList=new PublicController;
                $newAtList= $getNewAtList->getNewAtList($v['atlist']);
                $dynamic_list[$k]["atList"]=$newAtList;
            }
        }
    ...
    public function doSearch()
    {
        ...
        $count =M('Sof')->alias("s")->where($where)->count();
        $dynamic_list =M('Sof')->alias("s")->join("right join __USER__ as u on u.id = s.uid")->where($where)->field("s.id,u.avatar,s.atlist,s.uid,s.create_time,u.name,s.content,s.praise_num,s.click_num,s.reply_num,s.transfer_num,s.type")->page($page, $page_num)->order("s.create_time desc")->select();
        foreach ($dynamic_list as $k => $v) {
            //点赞
            $undigg = M('Like')->where(array("object_id" => $v["id"], "tablename" => "sof", "uid" => $uid))->field("undigg")->find();
            if ($undigg) {
    ...
    public function dynamicRecommend()
    {
        ...
        $dynamic_list =M('Sof')->alias("s")->join("right join __USER__ as u on u.id = s.uid")->where($where)->field("s.id,u.avatar,s.uid,s.atlist,s.create_time,s.transfer_id,s.transfer_sid,u.name,s.content,s.praise_num,s.click_num,s.reply_num,s.transfer_num,s.type")->page($page, $page_num)->order("s.create_time desc")->select();
    ...
    public function dynamicBuddy()
    {
        ...
        $dynamic_list =M('Sof')->alias("s")->join("right join __USER__ as u on u.id = s.uid")->where($where)->field("s.id,u.avatar,s.uid,s.atlist,s.create_time,u.name,s.content,s.praise_num,s.click_num,s.reply_num,s.transfer_num,s.type,s.transfer_sid")->page($page, $page_num)->order("s.create_time desc")->select();
    ...
    public function dynamicDetails()
    {
        ...
        $dynamic_detail =M('Sof')->alias("s")->join("right join __USER__ as u on u.id = s.uid")->where($where)->field("s.id as sid,s.uid,s.create_time,s.atlist,u.name,u.avatar,s.content,s.click_num,s.reply_num,s.type,s.transfer_num,s.transfer_id,s.transfer_sid")->order("s.create_time desc")->find();
        //点赞
        ...
        //获取用户@的好友的数据
        $getNewAtList=new PublicController;
        $newAtList= $getNewAtList->getNewAtList($dynamic_detail['atlist']);
        $dynamic_detail["atList"]=$newAtList;

        //获取用户@的好友的数据
        if (!empty($dynamic_detail['suid'])){
            $where = [
    ...
    public function replyReceived(){
        ...
        $reply_list = M('Reply')->alias("r")->where($where_reply)->join(' left join __USER__ u on u.id = r.uid')->field('r.object_id as sid, r.id as rid,r.create_time as reply_create_time, r.is_read,r.uid as reply_uid,r.content as reply_content,u.avatar as reply_user_avatar,u.name as reply_user_name')->page($page, $page_num)->select();
