...
class IndexController extends AppController
{
    ...
    public function __construct($address="0.0.0.0", $port="8761")
    {
...
//        json_decode($msg, true);
        ...
        if ($g['type'] == 'add') {
            switch ($g['code']){
                case $code['GET_USER_INFO']:$ar = $this->get_user_info($ar, $g['unionid']);break;
            }
    ...
    function getusers()
    {
        $ar = array();
        foreach ($this->users as $k => $v) {
            $ar[] = array('code' => $k, 'name' => $v['name'],'data' => ['user_info' =>$v['user_info']]);
        }
        return $ar;
    }
     ...
     * @param $unionid
     * User: 孙成睿
     * DateTime: 2020/8/1819:06
     * @return mixed
     * Return: @return
     */
    function get_user_info($ar,$unionid){
        if (empty($unionid)){
            $ar['msg'] = '非法请求';
        }

        $user_info = M('User')->where(['unionid'=>$unionid])->field('name,unionid')->find();

        if ($user_info){
            $ar['data']['user_info'] = $user_info;
            $ar['data']['user_info']['user_token'] = $user_info['unionid'].time();
        }else {
            $ar['msg'] = '用户不存在';
        }
        return $ar;
...
$sk = new IndexController('0.0.0.0' , "8761");
